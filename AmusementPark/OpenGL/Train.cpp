...
{
	trainModel = new Model("./Asset/Model/train v4.obj");
	trainHeadModel = new Model("./Asset/Model/trainHead v4.obj");
	trainWheelModel = new Model("./Asset/Model/wheel.obj");
...
{
	float arcLength = m_track->AddArcLen(m_trainU, num);
	m_trainU += arcLength;
	...
	//l = r * pi * THETA / 180
	wheelRotate += 180 * num / (2.5f * 3.1415926);
	if (wheelRotate >= 360.f)wheelRotate -= 360.f;
}

float Train::AddPhysics()
{
	m_orient = glm::normalize(m_orient);
	float sinTHETA = -m_orient.y;
	//a = g*sinTHETA
	//V = a*t
	//V = v0 + at
	float a = 9.8 * sinTHETA;
	return a;
}
...
void Train::DrawTrainHead(float trainU, Shader* shader)
{
	...
	shader->setFloat3("u_color", glm::vec3(0.6f, 0.6f, 0.6f));
	{
		...
		model = glm::translate(model, glm::vec3(trainPos0.x, trainPos0.y, trainPos0.z));
		model = model * rotate;
	...
	//wheel
	{
		for (int j = 0; j < 2; j++)
		{
			...
			{
				glm::mat4 model = glm::mat4(1.0f);
				model = glm::translate(model,trainPos0 + 0.6f * (- trainHead * (float)(i-1) + trainOrient * 0.25f + trainCross * 0.85f * (float)j));
				model = model * rotate;

				//glUniform1f(glGetUniformLocation(shader->ID, "scale"), 0.6f);
				model = glm::scale(model, glm::vec3(0.06f));
				model = glm::rotate(model, glm::radians(180.f), glm::vec3(0, 1, 0));
				model = glm::rotate(model, glm::radians(wheelRotate), glm::vec3(0, 0, 1));

				shader->setMat4("u_model", model);

				trainWheelModel->Draw(*shader);
			}
		}
	}
...
void Train::DrawTrainTruck(float trainU, Shader* shader)
{
	...
	//wheel
	{
		for (int j = 0; j < 2; j++)
		{
			...
			{
				glm::mat4 model = glm::mat4(1.0f);
				model = glm::translate(model, trainPos0 + 0.6f * (-trainHead * 0.5f * (float)(2 *i - 1) + trainOrient * 0.25f + trainCross * 0.85f * (float)j));
				model = model * rotate;
